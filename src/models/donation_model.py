...
class DonationModel(BaseModel):
    ...
    def getHistoryByUserIdWithDetails(self, userId, limit=50, offset=0):
        """
        Ambil history donasi berdasarkan user_id (created_by)
        dengan data campaign dan institusi
        
        Args:
            userId: User ID (format: 'user_123' atau 123)
            limit: Jumlah record yang diambil
            offset: Offset untuk pagination
        
        Returns:
            List of donations with campaign and institution details
        """
        # Normalize user_id format
        if isinstance(userId, int):
            created_by = f'user_{userId}'
        else:
            created_by = userId if userId.startswith('user_') else f'user_{userId}'
        ...
        with self.conn.cursor() as cursor:
            sql = f"""
                SELECT 
                    d.*,
                    c.nama_campaign,
                    c.deskripsi as campaign_deskripsi,
                    c.gambar as campaign_gambar,
                    c.institusi_id,
                    i.nama as institusi_nama,
                    i.logo as institusi_logo
                FROM {self.table_name} d
                LEFT JOIN adm_campaign c ON d.campaign_id = c.id
                LEFT JOIN adm_institusi i ON c.institusi_id = i.id
                WHERE d.created_by = %s 
                AND d.is_delete = 'N'
                ORDER BY d.created_date DESC
                LIMIT %s OFFSET %s
            """
            cursor.execute(sql, (created_by, limit, offset))
