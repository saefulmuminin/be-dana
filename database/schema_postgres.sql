...
SELECT 'DANA', 'e money', 'dana.png', 'DANA', 'emoney', 0, 10000, 50000000, 1, 0.000, '1. Pilih metode pembayaran DANA
2. Anda akan diarahkan ke halaman DANA
3. Login dengan akun DANA Anda
4. Konfirmasi pembayaran
5. Pembayaran berhasil', 'Y', 'N', NOW()
WHERE
    NOT EXISTS (
        SELECT 1
...
COMMENT ON
TABLE ref_dana_merchant_config IS 'Konfigurasi merchant DANA untuk pembayaran langsung';

COMMENT ON
TABLE trx_dana_payment IS 'Transaksi pembayaran DANA khusus merchant';

COMMENT ON
TABLE log_dana_webhook IS 'Log webhook/callback dari DANA';
